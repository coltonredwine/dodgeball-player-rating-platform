...
export default async function RaterPreviewPage({
  ...
  params: Promise<{ leagueSlug: string; raterId: string }>;
}) {
  const { leagueSlug, raterId } = await params;
  ...
  let rater;
  try {
    rater = await assertRaterInLeague(raterId, leagueId);
  } catch {
    redirect(leaguePath(league.slug, "/backend"));
  }

  const [submission, activePlayers] = await Promise.all([
    ...
    <main className="min-w-0 overflow-x-hidden bg-white text-zinc-900">
      ...
      <section className="mx-auto min-w-0 max-w-7xl space-y-4 px-3 py-6 sm:px-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <Link className="text-sm text-blue-700 underline" href={leaguePath(league.slug, "/backend")}>
              ← Back to admin
            </Link>
