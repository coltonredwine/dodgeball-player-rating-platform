...
export function LeaningIcon({ leaning, size = 14, className = "", monochrome = false }: Props) {
  const sharedClass = `inline-block shrink-0 align-middle ${className}`;
  const offensive = leaning === "offensive";
  const fill = monochrome ? "var(--draft-glyph-mono, #9ca3af)" : undefined;

  return offensive ? (
    <OffenseGlyph size={size} className={sharedClass} fill={fill} />
  ) : (
    <DefenseGlyph size={size} className={sharedClass} fill={fill} />
  );
}
